...
mod method_non_parametric_trait_impl {
    ...
    fn convert_to<TS, T: ConvertTo<TS>>(thing: T) -> TS {
        thing.convert_to() // $ method=ConvertTo::convert_to SPURIOUS: method=T::convert_to
    }

    fn thing<TP: MyTrait<S1>>(thing: TP) -> S1 {
        thing.convert_to() // $ method=T::convert_to
    }
    ...
    pub fn f() {
        ...
        let i = thing.convert_to(); // $ type=i:S1 method=T::convert_to
